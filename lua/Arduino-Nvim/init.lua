...
function M.create_floating_cli_monitor()
	...
	vim.api.nvim_buf_set_keymap(buf, "n", "<CR>", "<cmd>lua vim.api.nvim_win_close(" .. win .. ", false)<CR>", { noremap = true, silent = true })
	return buf, win, opts
end
...
function M.monitor()
	...
	vim.fn.jobstart(describe_cmd, {
		...
		on_exit = function()
			vim.api.nvim_buf_set_lines(buf, -1, -1, false, { "", "Starting monitor...", "Press CTRL-C or Esc to exit.", "" })
			...
			vim.fn.termopen(serial_command, {
				...
				on_exit = function(_, exit_code)
					if exit_code ~= 0 and vim.api.nvim_buf_is_valid(term_buf) then
						vim.api.nvim_buf_set_lines(term_buf, -1, -1, false, { "", "Monitor exited with code: " .. exit_code })
					end
				end,
			})
